...
async function main() {
	...
	let timeIntBetweenPayments = 1 //time interval between the payments done to heir
	...
	let timeToSet = currentTime() + 100000000;
    await setTime(ethers.provider, timeToSet);
	await pecunia_lock.withdrawAssets(owner.address, heir.address);
	await printBalances(owner, pecunia_lock, heir)
    await printNftOwner(owner_token, onft_tokenId, "owner nft") 
